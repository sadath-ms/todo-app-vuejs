...
const addTask = () => {
    if (newTask.value.trim !== '') {
        tasksList.value.push(newTask.value)
        newTask.value = ''
    }
}
...
<template>
    <div class="todo-app">
        ...
        <ul class="task-list">
            <li class="task-item" v-for="(name, index) in tasksList" :key="index">{{ name }} <button
                    class="remove-button" @click="removeTask"> Remove </button>
            </li>
        </ul>
